...
def _process_scan(raw):
    ...
    if new_scan == inversed_new_scan:                                   # [1/22]에 사업부장님께서 여기를 주석 처리 해보셨었음.
...
class RPLidar(object):
    ...
    def _read_descriptor(self):
        ...
        elif not descriptor.startswith(SYNC_BYTE + SYNC_BYTE2):                # 일단 len(descriptor)==DESCRIPTOR_LEN을 만족하는데
            raise RPLidarException('Incorrect descriptor starting bytes')      # descriptor가 SYNC_BYTE + SYNC_BYTE2로 시작하지 안는다는건데...
...
#                print(data_in_buf)
    ...
    def _initScan(self):
        cmd = SCAN_BYTE
        self._send_cmd(cmd)  # Demande de SCAN
        dsize, is_single, dtype = self._read_descriptor() # Réception du descripteur des données de SCAN
        if dsize != 5:
            raise RPLidarException('Wrong get_info reply length')
        if is_single:
            raise RPLidarException('Not a multiple response mode')
    ...
    def scanAllDataFromLidar(self):
        dsize = self._initScan()
        scan = []
        while True:
            raw = self._read_response(dsize)
    ...
    def scan2DataFromLidar(self,minRightAngle,maxRightAngle,minLeftAngle,maxLeftAngle):
        dsize = self._initScan()
        scanLeft = []
        scanRight = []
        while True:
            ...
            new_scan, quality, angle, distance = _process_scan(raw)
            if new_scan:
                yield scanRight,scanLeft
                scanLeft = []
                scanRight = []
            else:
                if quality > 0 and distance > 0:
                    if minRightAngle <= angle <= maxRightAngle:
                        scanRight.append((quality, angle, distance))
                    elif minLeftAngle <= angle <= maxLeftAngle:
                        scanLeft.append((quality, angle, distance))

    def scan2ArrayFromLidar(self,minRightAngle,maxRightAngle,minLeftAngle,maxLeftAngle):
        dsize = self._initScan()
        radiusLeft = np.array([])
        thetaLeft = np.array([])
        radiusRight = np.array([])
        thetaRight = np.array([])
        while True:
            raw = self._read_response(dsize)
            new_scan, quality, angle, distance = _process_scan(raw)
            if new_scan:
                yield radiusRight,thetaRight,radiusLeft,thetaLeft
                radiusLeft = np.array([])
                thetaLeft = np.array([])
                radiusRight = np.array([])
                thetaRight = np.array([])
                ...
                if quality > 0 and distance > 0:
                    if minRightAngle <= angle <= maxRightAngle:
                        thetaRight=np.append(thetaRight,[angle])
                        radiusRight=np.append(radiusRight,[distance])
                    elif minLeftAngle <= angle <= maxLeftAngle:
                        thetaLeft=np.append(thetaLeft,[angle])
                        radiusLeft=np.append(radiusLeft,[distance])
